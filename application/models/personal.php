...
class Personal extends CI_Model{
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (id_persona,numero_ci,nombre_persona,apellido_persona,correo,fecha_nacimiento,direccion,id_sexo,id_cargo,id_usuario,estado_per) ";
		$sql .= "values (nextval('sq_persona'),'$this->numero_ci','$this->nombre_persona','$this->apellido_persona','$this->correo','$this->fecha_nacimiento','$this->direccion',$this->id_sexo,'$this->id_cargo','$this->id_usuario',1)";
...
// partiendo de que ya tenemos creado un objecto personal previamente utilizamos el contexto
	...
	public function update_estado_per($id,$estado){
		$sql = "update ".self::$tablename." set estado_per=$estado where id_persona=$id";
		return $query = array($this->db->query($sql),'');
	}
