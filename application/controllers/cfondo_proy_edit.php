...
class Cfondo_proy_edit extends C_datos {
	...
	{ 
		...
		if($datosUsuario['fondos']==null){
		 ...
		 $this->agregarNuevoIdFondo($id_p);	
		  ...
		  $this->load->view('vsisinf/proyecto/vpresupuesto_proy_edit');
		}	
   ...
   function completarDatos($id,$ca){
		...
		if($datos['fondos']!=null){
			if($ca==0){
	    	 $datos['ep']=$this->ep->getAllByIdFondoEdit($datos['fondos']->id_proy_ep);
	    	 $datos['cambioMon']=null;
	    	}else{
	    	 foreach ($datos['cambio']->result() as $cam) {
	    		if($ca==$cam->id_cambio){
	    			$datos['ep']=$this->ep->getAllByIdFondoCambioEdit($datos['fondos']->id_proy_ep,$cam->valor);
	    			$datos['cambioMon']=$cam;
	    			break;
	    		}
	    	 }
