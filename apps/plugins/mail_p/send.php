...
function sendemail($mail_username,$mail_userpassword,$mail_setFromEmail,$mail_setFromName,$mail_addAddress,$txt_message,$mail_subject, $template){
	...
	if(!$mail->send()) {
		echo "<script>$('#mensajeR').append('<label class=\'text-danger\'>Ocurrio un error! al enviar los datos, vuelva a registrarlos :)</label>');$('#mensajeR').hide().fadeIn(1500);</script>";
return false;
	} else {
		echo "<script>$('#mensajeR').append('<label class=\'text-success\'>Usuario Registrado con exito! Le enviamos un correo, con el usuario y contraseña...</label>');$('#mensajeR').hide().fadeIn(1500);</script>";
	return true;
	}
}
?>