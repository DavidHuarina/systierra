...
<body class="solImp">
  <footer>
  <center><label id="footer_texto"><?=strftime('%d de %B de %Y',time())?></label></center><!--<img class="logo-pie" src="imagenes/logo_tierra1.png">-->   
  </footer>
  <div class="cont">
           <div class="row">
               <div class="card">
                       ...
                       <table class="table text-small table-bordered">
                        <tr><td colspan="4" align="center"><h5><b>Datos del Reporte</b></h5></td></tr>
                          <tr><td><b>Proyectos</b></td><td><?=$nreg?></td><td colspan="2"><?php foreach ($proy->result() as $pro) {
                            echo "- ".utf8_decode($pro->nombre_proyecto)."<br><br>";
                          }?></td></tr> 
                       ...
                       <table class="table text-small text-white">
                          ...
                          $tipoAct=$this->tipoact->getById($i+1); 
                            if($tt[$i]!=0){
                              $sumtt=$sumtt+$tt[$i];
                              ?>
                             <tr><td><?=utf8_decode($tipoAct->tipo_nom)?></td><td><?=$tt[$i]?></td></tr>
                            <?php
                            }
                          }
                          ?> 
                     ...
                     <div class="col-md-8 bg-amarillo text-white grow">
                        ...
                        <table class="table text-small text-white">
                          ...
                          $ttm=$ttm+$pm[$tipop->id_tipopar-1];
                             ?><tr><td><?=utf8_decode($tipop->nombre_tipopar)?></td><td><?=$ph[$tipop->id_tipopar-1]?></td><td><?=$pm[$tipop->id_tipopar-1]?></td><td><?=$ph[$tipop->id_tipopar-1]+$pm[$tipop->id_tipopar-1]?></td></tr><?php
                  ...
                  <div class="table-responsive-sm">
                    ...
                    $cont=0;         
                      foreach ($acti->result() as $ac) {
                       ...
                       <tr class="text-small">
                        <td><?=$cont?></td>
                         <td><?=utf8_decode($ac->nombre_proyecto)?></td>
                         <td><?php $porciones = explode("@", $ac->sub_nom);?> <?=utf8_decode($porciones[0])?></td>
                         <td><?=$ac->act_dias?></td>
                         <td><?=$persona->nombre_persona?> <?=$persona->apellido_persona?></td>
                         <td><?=utf8_decode($ac->tipo_nom)?></td>
